...
export const StoryNebulaRenderer = ({ currentQ, handleAnswer, feedback, gameTheme }) => {
    ...
    <div className="w-full max-w-3xl">
      ...
      <div className="grid grid-cols-1 gap-3 relative z-20">
        {options.map((opt, i) => (
          <button
            key={i}
            onClick={() => handleAnswer(opt, currentQ.option1)}
            className={`p-4 rounded-xl text-left font-medium text-lg transition-all transform hover:scale-102 hover:shadow-xl ${
              feedback
...
export const InferenceInvestigatorRenderer = ({ currentQ, handleAnswer, feedback, gameTheme }) => {
  const options = [currentQ.option1, currentQ.option2, currentQ.option3, currentQ.option4].filter(Boolean);
      ...
      <div className="grid grid-cols-1 gap-3 relative z-20">
        {options.map((opt, i) => (
          <button
            ...
            className={`p-4 rounded-xl text-left font-medium text-lg transition-all transform hover:scale-102 hover:shadow-xl group ${
              feedback
                ? opt === currentQ.answer
                  ? 'bg-green-500 text-white border-2 border-green-300 shadow-lg shadow-green-500/50'
                  : 'bg-gray-700 text-gray-400 border-2 border-gray-600'
