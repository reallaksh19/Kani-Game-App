...
export interface SupabaseGuardianIdentityOptions {
  supabaseUrl: string;
...
/** Provider-specific browser adapter. No product/domain module should import this directly. */
export class SupabaseGuardianIdentityProvider implements GuardianIdentityProvider {
  ...
  constructor(options: SupabaseGuardianIdentityOptions) {
  ...
  private async tokenRequest(grantType: 'password' | 'refresh_token', body: Record<string, string>): Promise<StoredGuardianAuth> {
    ...
    try {
      ...
      throw new GuardianIdentityError(error instanceof Error ? error.message : 'Guardian authentication request failed.', {
        status: 0,
        code: 'NETWORK_ERROR',
      });
    }
    ...
    try {
      ...
      // Auth provider normally returns JSON; preserve a bounded generic error otherwise.
    }
    if (!response.ok) {
      throw new GuardianIdentityError(messageFromBody(payload, 'Guardian sign-in failed.'), {
    ...
    if (!accessToken || !refreshToken || !userId || !email) {
      throw new GuardianIdentityError('Guardian auth provider returned an incomplete session.', {
        status: 502,
  ...
  async signIn(email: string, password: string): Promise<GuardianAccount> {
    ...
    if (!normalizedEmail || !password) {
      throw new GuardianIdentityError('Email and password are required.', { status: 400, code: 'INVALID_INPUT' });
    }
