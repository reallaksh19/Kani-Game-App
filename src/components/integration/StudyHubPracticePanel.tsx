...
import { KaniCatalogPage, StudyHubPageDocument } from '../../integration/kani/contracts';
...
export const StudyHubPracticePanel: React.FC<StudyHubPracticePanelProps> = ({ page, pageMeta }) => {
  ...
  if (adapted.questions.length === 0 && adapted.unsupported.length === 0) {
    return (
      <div className="mt-5 rounded-2xl border border-slate-700 bg-slate-900/60 p-4 text-sm text-slate-300">
        This Study-Hub page has no structured practice questions. Lesson rendering remains in Study-Hub.
      </div>
    );
  }

  if (mode === 'running' && activeStudent) {
    return (
    ...
    return (
      <section className="mt-6 rounded-3xl border border-emerald-300/25 bg-emerald-950/20 p-5">
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          ...
          <button onClick={() => setMode('summary')} className="rounded-full border border-slate-500 px-5 py-2.5 font-bold text-slate-200 hover:bg-slate-800">Back to page</button>
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="mt-6 rounded-3xl border border-cyan-300/25 bg-cyan-950/20 p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-300">
            {adapted.questions.length} supported question{adapted.questions.length === 1 ? '' : 's'} will use Kani's active student, Randomise setting, timing, review and canonical attempt store.
          </p>
        </div>
        <button
          type="button"
          disabled={!activeStudent || adapted.questions.length === 0}
      ...
      {!activeStudent && <div className="mt-3 text-sm text-rose-200">A stable Kani student profile is required to record practice attempts.</div>}
    </section>
  );
};
